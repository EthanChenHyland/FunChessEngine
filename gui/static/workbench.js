...
function wbFilters() {
  ...
  if($('wbMissing').value) filters.missing=$('wbMissing').value;
  if($('wbExactPlayers').checked) filters.exact_players=true;
...
function wbApplyFilters(filters) {
  ...
  $('wbMissing').value=filters.missing || '';
  $('wbExactPlayers').checked=Boolean(filters.exact_players);
...
function wbRenderPreview() {
  ...
  for(const square of wbBoardSquares(current?.fen || STARTING_FEN,wb.flipped)) {
    ...
    if($('wbCoordinates').checked && ((typeof wsPrefs==='function' && wsPrefs().coordsMode==='all') || square.row===7 || square.col===0)) cell.append(wbElement('small',square.square));

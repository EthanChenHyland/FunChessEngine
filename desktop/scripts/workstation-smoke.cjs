...
module.exports=async function workstationSmoke(window,waitFor) {
  await window.webContents.executeJavaScript(String.raw`(async()=>{
    ...
    check(exports.some(item=>item.name.endsWith('.pgn') && item.text.includes('[White')),'Single game PGN export missing');check(exports.some(item=>item.name.endsWith('.txt') && item.text.includes('Reference #')),'Citations export missing');const customization=exports.find(item=>item.name.endsWith('customization.json'));check(JSON.parse(customization.text).settings.targetStyle==='ring' && JSON.parse(customization.text).appearance.theme,'Customization export missing settings');await wsImportCustomization({size:customization.text.length,text:async()=>customization.text});

...
module.exports=async function productivitySmoke(window,waitFor) {
  await window.webContents.executeJavaScript(`(async()=>{
    ...
    if(wb.pageSnapshot.pageGames!==25 || wb.pageSnapshot.total!==34 || Math.round(wb.pageSnapshot.averageElo)!==2350 || !document.getElementById('wbSnapshotEco').textContent.includes('C20 · 25'))throw new Error('Current result snapshot failed');
    document.getElementById('wbColumnsEssential').click();if(!document.querySelector('[data-wb-column="event"]').hidden)throw new Error('Essential column preset failed');document.getElementById('wbStickyPlayers').checked=true;document.getElementById('wbStickyPlayers').dispatchEvent(new Event('change'));document.getElementById('wbWrapCells').checked=true;document.getElementById('wbWrapCells').dispatchEvent(new Event('change'));document.getElementById('wbDensity').value='spacious';document.getElementById('wbDensity').dispatchEvent(new Event('change'));if(document.getElementById('wbTable').dataset.wbStickyPlayers!=='true' || document.getElementById('wbTable').dataset.wbWrap!=='true' || document.getElementById('wbTable').dataset.wbDensity!=='spacious')throw new Error('Persistent table layout failed');document.getElementById('wbColumnsAll').click();document.getElementById('wbStickyPlayers').checked=false;document.getElementById('wbStickyPlayers').dispatchEvent(new Event('change'));document.getElementById('wbWrapCells').checked=false;document.getElementById('wbWrapCells').dispatchEvent(new Event('change'));document.getElementById('wbDensity').value='comfortable';document.getElementById('wbDensity').dispatchEvent(new Event('change'));
